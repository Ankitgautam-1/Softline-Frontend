...
export default function Table() {
  ...
  const activeStyle = { ...stateStyle, backgroundColor: "#06bb30" };

  const expiringStyle = {
    ...stateStyle,
    backgroundColor: "#f79c15",
  };
  const expiredStyle = {
    ...stateStyle,
  ...
  const columns: GridColumns<Contract> = [
    {
      field: "actions",
      headerName: "Actions",
      width: 130,
      ...
      renderCell(params: GridCellParams) {
        ...
        return params.value === "Expired" ? (
          <div style={containerStyle}>
            <span style={expiredStyle}>{params.value}</span>
          </div>
        ) : params.value === "Expiring" ? (
          <div style={containerStyle}>
            <span style={expiringStyle}>{params.value}</span>
          </div>
        ) : currentDate.isAfter(startDate) ? (
          <div style={containerStyle}>
            <span style={activeStyle}>{params.value}</span>
          </div>
        ) : (
          <div style={containerStyle}>
            <span style={activeStyle}>Not Started</span>
          </div>
        );
      },
    },
